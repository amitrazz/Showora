            ...
            {customer.purchases.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {customer.purchases.map((purchase: any) => (
                  <Card key={purchase.id} className="shadow-sm border-border/50 bg-card hover:bg-muted/10 transition-colors">
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-start">
                        <span className="font-mono text-xs font-semibold text-primary">
                          #{purchase.id.substring(0, 8).toUpperCase()}
                        </span>
                        <Badge variant="outline" className={purchase.status === 'Completed' ? 'text-emerald-500 bg-emerald-500/10' : ''}>
                          {purchase.status}
                        </Badge>
                      </div>
                    ...
                    <CardContent className="pt-2 text-sm space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono font-medium">{formatCurrency(purchase.amount)}</span>
                      </div>
                      <div className="flex justify-between text-xs text-muted-foreground pt-2 border-t border-border/50">
                        <span>Date: {format(new Date(purchase.date), 'dd MMM yyyy')}</span>
                        <Link to="/sales/$saleId" params={{ saleId: purchase.id }} className="text-primary hover:underline font-medium">
                          View Details →
                        </Link>
                      </div>
                    </CardContent>
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            {customer.payments.length === 0 ? (
              ...
              <div className="space-y-3">
                {customer.payments.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((payment: any) => (
